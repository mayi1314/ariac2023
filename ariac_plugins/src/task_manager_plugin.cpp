...
namespace ariac_plugins {
    ...
    public:
        ...
        //============== SERVICES =================
        rclcpp::Service<ariac_msgs::srv::AgvPartPose>::SharedPtr agv1_sensor_srv_{nullptr};
        rclcpp::Service<ariac_msgs::srv::AgvPartPose>::SharedPtr agv2_sensor_srv_{nullptr};
        ...
        int
        ScoreQuadrant(bool is_correct_part_type, bool is_correct_part_color, bool is_flipped_part, bool is_faulty_part);
        ...
        CheckQuadrantQuality(int quadrant, ariac_common::KittingTask task, ariac_common::KittingShipment shipment,
                             std::string order_id);
    ...
    TaskManagerPlugin::Load(gazebo::physics::WorldPtr _world, sdf::ElementPtr _sdf) {
        ...
        impl_->agv1_tray_sub_ = impl_->gznode_->Subscribe(tray1_topic,
                                                          &TaskManagerPluginPrivate::AGV1TraySensorCallback,
                                                          impl_.get());
        impl_->agv2_tray_sub_ = impl_->gznode_->Subscribe(tray2_topic,
                                                          &TaskManagerPluginPrivate::AGV2TraySensorCallback,
                                                          impl_.get());
        impl_->agv3_tray_sub_ = impl_->gznode_->Subscribe(tray3_topic,
                                                          &TaskManagerPluginPrivate::AGV3TraySensorCallback,
                                                          impl_.get());
        impl_->agv4_tray_sub_ = impl_->gznode_->Subscribe(tray4_topic,
                                                          &TaskManagerPluginPrivate::AGV4TraySensorCallback,
                                                          impl_.get());
        ...
        impl_->station1_sub_ = impl_->gznode_->Subscribe(station1_topic,
                                                         &TaskManagerPluginPrivate::Station1SensorCallback,
                                                         impl_.get());
        impl_->station2_sub_ = impl_->gznode_->Subscribe(station2_topic,
                                                         &TaskManagerPluginPrivate::Station2SensorCallback,
                                                         impl_.get());
        impl_->station3_sub_ = impl_->gznode_->Subscribe(station3_topic,
                                                         &TaskManagerPluginPrivate::Station3SensorCallback,
                                                         impl_.get());
        impl_->station4_sub_ = impl_->gznode_->Subscribe(station4_topic,
                                                         &TaskManagerPluginPrivate::Station4SensorCallback,
                                                         impl_.get());
        ...
        impl_->sensor_health_pub_ = impl_->ros_node_->create_publisher<ariac_msgs::msg::Sensors>("/ariac/sensor_health",
                                                                                                 10);
        impl_->robot_health_pub_ = impl_->ros_node_->create_publisher<ariac_msgs::msg::Robots>("/ariac/robot_health",
                                                                                               10);
        impl_->competition_state_pub_ = impl_->ros_node_->create_publisher<ariac_msgs::msg::CompetitionState>(
    ...
    //==============================================================================
    void TaskManagerPlugin::AgvSensorSrv(ariac_msgs::srv::AgvPartPose::Request::SharedPtr request,
                                          ariac_msgs::srv::AgvPartPose::Response::SharedPtr response) {
        int agv = request->id;
        KDL::Frame world_to_sensor;
        tf2::fromMsg(gazebo_ros::Convert<geometry_msgs::msg::Pose>(
                             gazebo::msgs::ConvertIgn(impl_->agv_tray_images_[agv].pose())),
                     world_to_sensor);

        for (int i = 0; i < impl_->agv_tray_images_[agv].model_size(); i++) {
            const auto &lc_model = impl_->agv_tray_images_[agv].model(i);
            std::string model_name = lc_model.name();

            bool classified_part = false;
            // Determine part type
            for (const auto &part_type: impl_->part_types_) {
                if (model_name.find(part_type.first) != std::string::npos) {
                    // Determine part color
                    for (const auto &color: impl_->part_colors_) {
    ...
    TaskManagerPlugin::BuildAssemblyTaskMsg(std::shared_ptr <ariac_common::AssemblyTask> _task) {
    ...
    TaskManagerPlugin::BuildCombinedTaskMsg(std::shared_ptr <ariac_common::CombinedTask> _task) {
        ...
        for (const auto &order: impl_->on_part_placement_orders_) {
            ...
            if (!order->IsAnnounced()) {
                ...
                for (const auto &agv_part: parts) {
                    ...
                    if (agv_part.GetType() == part_condition->GetType() &&
                        agv_part.GetColor() == part_condition->GetColor()) {
                        auto order_message = BuildOrderMsg(order);
    ...
    void TaskManagerPlugin::ProcessOnSubmissionOrders() {
        for (const auto &order: impl_->on_order_submission_orders_) {
            if (!order->IsAnnounced()) {
                ...
                if (std::find(impl_->submitted_orders_.begin(), impl_->submitted_orders_.end(), trigger_order) !=
                    impl_->submitted_orders_.end()) {
        ...
        for (const auto &robot_malfunction: impl_->in_progress_robot_malfunctions_) {
            if (robot_malfunction->HasStarted() && !robot_malfunction->HasCompleted()) {
                ...
                if (impl_->elapsed_time_ >= start_time + duration) {
                    RCLCPP_INFO_STREAM(impl_->ros_node_->get_logger(),
                                       "Stopping robot malfunction challenge at time: " << impl_->elapsed_time_);
                    robot_malfunction->SetCompleted();

                    impl_->ceiling_robot_health_ = true;
                    impl_->floor_robot_health_ = true;
                    // remove the challenge from the list
                    impl_->in_progress_robot_malfunctions_.erase(
                            std::remove(impl_->in_progress_robot_malfunctions_.begin(),
    ...
    void TaskManagerPlugin::ProcessInProgressSensorBlackouts() {
        for (const auto &sensor_blackout: impl_->in_progress_sensor_blackouts_) {
            if (sensor_blackout->HasStarted() && !sensor_blackout->HasCompleted()) {
                ...
                if (impl_->elapsed_time_ >= start_time + duration) {
                    RCLCPP_INFO_STREAM(impl_->ros_node_->get_logger(),
                                       "Stopping sensor blackout challenge at time: " << impl_->elapsed_time_);
    ...
    void TaskManagerPlugin::ProcessTemporalRobotMalfunctions() {
        for (const auto &robot_malfunction: impl_->time_based_robot_malfunctions_) {
            if (impl_->elapsed_time_ >= robot_malfunction->GetTriggerTime() && !robot_malfunction->HasStarted()) {
                RCLCPP_INFO_STREAM(impl_->ros_node_->get_logger(),
                                   "Starting robot malfunction challenge at time: " << impl_->elapsed_time_);
    ...
    TaskManagerPlugin::ProcessOnSubmissionSensorBlackouts() {
        for (const auto &sb: impl_->on_submission_sensor_blackouts_) {
            ...
            if (std::find(impl_->submitted_orders_.begin(), impl_->submitted_orders_.end(), trigger_order) !=
                ...
                if (!sb->HasStarted()) {
                    RCLCPP_INFO_STREAM(impl_->ros_node_->get_logger(),
                                       "Starting sensor blackout challenge at time: " << impl_->elapsed_time_);
    ...
    void TaskManagerPlugin::OnUpdate() {
        ...
        if (!impl_->agv1_sensor_srv_)
        impl_->agv1_sensor_srv_ = impl_->ros_node_->create_service<ariac_msgs::srv::AgvPartPose>(
                "/ariac/agv1_sensor", std::bind(&TaskManagerPlugin::AgvSensorSrv,
                                                this, std::placeholders::_1, std::placeholders::_2)
        );
        if (!impl_->agv2_sensor_srv_)
        impl_->agv2_sensor_srv_ = impl_->ros_node_->create_service<ariac_msgs::srv::AgvPartPose>(
                "/ariac/agv2_sensor", std::bind(&TaskManagerPlugin::AgvSensorSrv,
                                                this, std::placeholders::_1, std::placeholders::_2)
        );
        ...
        if (!impl_->start_competition_service_ && current_sim_time.Double() >= 5.0) {
            ...
            impl_->start_competition_service_ = impl_->ros_node_->create_service<std_srvs::srv::Trigger>(
                    "/ariac/start_competition",
                    std::bind(&TaskManagerPlugin::StartCompetitionServiceCallback, this, std::placeholders::_1,
                              std::placeholders::_2));
    ...
    void TaskManagerPlugin::OnTrialCallback(const ariac_msgs::msg::Trial::SharedPtr _msg) {
        ...
        impl_->total_orders_ = impl_->time_based_orders_.size() + impl_->on_part_placement_orders_.size() +
                               impl_->on_order_submission_orders_.size();
    ...
    //==============================================================================
    std::shared_ptr <ariac_common::KittingTask>
    TaskManagerPlugin::BuildKittingTask(const ariac_msgs::msg::KittingTask &_kitting_task) {
    ...
    //==============================================================================
    std::shared_ptr <ariac_common::AssemblyTask>
    TaskManagerPlugin::BuildAssemblyTask(const ariac_msgs::msg::AssemblyTask &_assembly_task) {
    ...
    //==============================================================================
    std::shared_ptr <ariac_common::CombinedTask>
    TaskManagerPlugin::BuildCombinedTask(const ariac_msgs::msg::CombinedTask &_combined_task) {
    ...
    void TaskManagerPlugin::StoreOrders(const std::vector <std::shared_ptr<ariac_msgs::msg::OrderCondition>> &orders) {
        for (auto order: orders) {
            ...
            if (order->condition.type == ariac_msgs::msg::Condition::TIME) {
                ...
                auto order_instance = std::make_shared<ariac_common::OrderTemporal>(order_id, order_type,
                                                                                    order_priority, impl_->time_limit_,
                                                                                    announcement_time);
                // Set the task
                if (kitting_task)
                ...
                auto part = std::make_shared<ariac_common::Part>(order->condition.part_place_condition.part.color,
                                                                 order->condition.part_place_condition.part.type);
                auto order_instance = std::make_shared<ariac_common::OrderOnPartPlacement>(order_id, order_type,
                                                                                           order_priority,
                                                                                           impl_->time_limit_, agv,
                                                                                           part);
                // Set the task
                if (kitting_task)
                    order_instance->SetKittingTask(kitting_task);
                ...
                auto order_instance = std::make_shared<ariac_common::OrderOnSubmission>(order_id, order_type,
                                                                                        order_priority,
                                                                                        impl_->time_limit_,
                                                                                        submitted_order_id);
                // Set the task
                ...
                RCLCPP_ERROR_STREAM(impl_->ros_node_->get_logger(),
                                    "Unknown condition type: " << int(order->condition.type));
            }
        }
    }
    ...
    //==============================================================================
    void
    TaskManagerPlugin::StoreRobotMalfunctionChallenges(const ariac_msgs::msg::RobotMalfunctionChallenge &_challenge) {
        ...
        if (condition.type == ariac_msgs::msg::Condition::TIME) {
            ...
            auto robot_malfunction = std::make_shared<ariac_common::RobotMalfunctionTemporal>(duration,
                                                                                              robots_to_disable_vect,
                                                                                              trigger_time);
            ...
            auto part = std::make_shared<ariac_common::Part>(condition.part_place_condition.part.color,
                                                             condition.part_place_condition.part.type);
            auto robot_malfunction = std::make_shared<ariac_common::RobotMalfunctionOnPartPlacement>(duration,
                                                                                                     robots_to_disable_vect,
                                                                                                     part, agv);
            ...
            auto robot_malfunction = std::make_shared<ariac_common::RobotMalfunctionOnSubmission>(duration,
                                                                                                  robots_to_disable_vect,
                                                                                                  submitted_order_id);
    ...
    void TaskManagerPlugin::StoreSensorBlackoutChallenges(const ariac_msgs::msg::SensorBlackoutChallenge &_challenge) {
        ...
        if (condition.type == ariac_msgs::msg::Condition::TIME) {
            auto trigger_time = condition.time_condition.seconds;
            auto sensor_blackout = std::make_shared<ariac_common::SensorBlackoutTemporal>(duration,
                                                                                          sensors_to_disable_vect,
                                                                                          trigger_time);
            ...
            auto part = std::make_shared<ariac_common::Part>(condition.part_place_condition.part.color,
                                                             condition.part_place_condition.part.type);
            auto sensor_blackout = std::make_shared<ariac_common::SensorBlackoutOnPartPlacement>(duration,
                                                                                                 sensors_to_disable_vect,
                                                                                                 part, agv);
            impl_->on_part_placement_sensor_blackouts_.push_back(sensor_blackout);
        } else if (condition.type == ariac_msgs::msg::Condition::SUBMISSION) {
            auto submitted_order_id = condition.submission_condition.order_id;
            auto sensor_blackout = std::make_shared<ariac_common::SensorBlackoutOnSubmission>(duration,
                                                                                              sensors_to_disable_vect,
                                                                                              submitted_order_id);
    ...
    void TaskManagerPlugin::StoreHumanChallenge(const ariac_msgs::msg::HumanChallenge &_challenge) {
        ...
        if (condition.type == ariac_msgs::msg::Condition::TIME) {
            ...
            auto submitted_order_id = condition.submission_condition.order_id;
            auto human_challenge = std::make_shared<ariac_common::HumanChallengeOnSubmission>(behavior,
                                                                                              submitted_order_id);
    ...
    int
    TaskManagerPluginPrivate::ScoreQuadrant(bool is_correct_part_type, bool is_correct_part_color, bool is_flipped_part,
                                            bool is_faulty_part) {
    ...
    void TaskManagerPluginPrivate::ScoreKittingTask(std::shared_ptr <ariac_common::Order> _order,
                                                    std::string _submitted_order_id) {
        ...
        for (auto product: kitting_task->GetProducts()) {
            for (auto tray_part: shipment.GetTrayParts()) {
                if (tray_part.GetQuadrant() == product.GetQuadrant()) {
                    ...
                    quadrant_scores[quadrant] = ScoreQuadrant(is_correct_part_type, is_correct_part_color,
                                                              is_flipped_part, is_faulty_part);

                    // RCLCPP_INFO_STREAM(ros_node_->get_logger(), "Score: " << quadrant_scores[quadrant]);

                    auto quadrant_ptr = std::make_shared<ariac_common::Quadrant>(quadrant, is_correct_part_type,
                                                                                 is_correct_part_color, is_faulty_part,
                                                                                 is_flipped_part,
    ...
    //==============================================================================
    void TaskManagerPluginPrivate::ScoreAssemblyTask(std::shared_ptr <ariac_common::Order> _order,
                                                     std::string _submitted_order_id) {
        // These shared pointers are uniquely for KittingScore
                ...
                if (is_correct_part_type) {
                    ...
                    if (order_product.GetPart().GetType() == ariac_msgs::msg::Part::BATTERY) {
                        ...
                        battery_ptr = std::make_shared<ariac_common::ScoredAssemblyPart>(
                                shipment_part, is_correct_part_color, is_correct_pose, part_position, part_orientation,
                                part_score, faulty_part);
                    } else if (order_product.GetPart().GetType() == ariac_msgs::msg::Part::PUMP) {
                        bool faulty_part = false;
                        pump_ptr = std::make_shared<ariac_common::ScoredAssemblyPart>(shipment_part,
                                                                                      is_correct_part_color,
                                                                                      is_correct_pose, part_position,
                                                                                      part_orientation, part_score,
                                                                                      faulty_part);
                    } else if (order_product.GetPart().GetType() == ariac_msgs::msg::Part::REGULATOR) {
                        bool faulty_part = false;
                        regulator_ptr = std::make_shared<ariac_common::ScoredAssemblyPart>(shipment_part,
                                                                                           is_correct_part_color,
                                                                                           is_correct_pose,
                                                                                           part_position,
                                                                                           part_orientation, part_score,
                        ...
                        bool faulty_part = false;
                        sensor_ptr = std::make_shared<ariac_common::ScoredAssemblyPart>(shipment_part,
                                                                                        is_correct_part_color,
                                                                                        is_correct_pose, part_position,
                                                                                        part_orientation, part_score,
                                                                                        faulty_part);
    ...
    void TaskManagerPluginPrivate::PrintAssemblyScore(std::shared_ptr <ariac_common::AssemblyScore> assembly_score) {
        ...
        if (battery != nullptr) {
            ...
            output += "Position: " + std::to_string(battery->GetPosition().x) + ", " +
                      std::to_string(battery->GetPosition().y) + ", " + std::to_string(battery->GetPosition().z) + "\n";
            output += "Orientation: " + std::to_string(battery->GetOrientation().x) + ", " +
                      std::to_string(battery->GetOrientation().y) + ", " + std::to_string(battery->GetOrientation().z) +
                      "\n";
        ...
        if (pump != nullptr) {
            ...
            output += "Position: " + std::to_string(pump->GetPosition().x) + ", " +
                      std::to_string(pump->GetPosition().y) + ", " + std::to_string(pump->GetPosition().z) + "\n";
            output += "Orientation: " + std::to_string(pump->GetOrientation().x) + ", " +
                      std::to_string(pump->GetOrientation().y) + ", " + std::to_string(pump->GetOrientation().z) + "\n";
        ...
        if (regulator != nullptr) {
            ...
            output += "Position: " + std::to_string(regulator->GetPosition().x) + ", " +
                      std::to_string(regulator->GetPosition().y) + ", " + std::to_string(regulator->GetPosition().z) +
                      "\n";
            output += "Orientation: " + std::to_string(regulator->GetOrientation().x) + ", " +
                      std::to_string(regulator->GetOrientation().y) + ", " +
                      std::to_string(regulator->GetOrientation().z) + "\n";
        ...
        if (sensor != nullptr) {
            ...
            output += "Position: " + std::to_string(sensor->GetPosition().x) + ", " +
                      std::to_string(sensor->GetPosition().y) + ", " + std::to_string(sensor->GetPosition().z) + "\n";
            output += "Orientation: " + std::to_string(sensor->GetOrientation().x) + ", " +
                      std::to_string(sensor->GetOrientation().y) + ", " + std::to_string(sensor->GetOrientation().z) +
                      "\n";
    ...
    //==============================================================================
    void TaskManagerPluginPrivate::ScoreCombinedTask(std::shared_ptr <ariac_common::Order> _order,
                                                     std::string _submitted_order_id) {
    ...
    bool TaskManagerPlugin::SubmitOrderServiceCallback(
        ...
        if (std::find(impl_->trial_orders_.begin(), impl_->trial_orders_.end(), submitted_order_id) !=
            impl_->trial_orders_.end()) {
            ...
            for (auto &order: impl_->all_orders_) {
                ...
                impl_->trial_orders_.erase(
                        std::remove(impl_->trial_orders_.begin(), impl_->trial_orders_.end(), submitted_order_id),
                        impl_->trial_orders_.end());
            }
        } else {
            response->success = false;
    ...
    void TaskManagerPluginPrivate::PrintTrialScore(
        ...
        auto trial_completion_time = std::max(
                {kitting_submitted_time, assembly_submitted_time, combined_submitted_time});
    ...
    //==============================================================================
    std::array<bool, 4>
    TaskManagerPluginPrivate::HandleFaultyParts(std::shared_ptr <ariac_common::FaultyPartChallenge> _challenge,
                                                ariac_common::KittingTask _task,
                                                ariac_common::KittingShipment _shipment) {
                                ...
                                if (tray_part.GetQuadrant() == i) {
                                    ...
                                    if (tray_part.isCorrectType(product.GetPart().GetType()) &&
                                        ...
                                        RCLCPP_INFO_STREAM(ros_node_->get_logger(),
                                                           "Part in quadrant " << std::to_string(i) << " is faulty");
    ...
    //==============================================================================
    ariac_common::AssemblyShipment
    TaskManagerPluginPrivate::ParseAssemblyStationImage(gazebo::msgs::LogicalCameraImage &_msg) {
    ...
    ariac_msgs::msg::QualityIssue TaskManagerPluginPrivate::CheckQuadrantQuality(int quadrant,
        ...
        if (issue.missing_part || issue.incorrect_part_type || issue.incorrect_part_color || issue.faulty_part ||
            issue.flipped_part)
